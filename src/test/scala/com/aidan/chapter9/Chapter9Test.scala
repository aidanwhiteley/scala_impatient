...
class Chapter9Test extends FunSuite {
  ...
  test("Find class files in all subdirectories") {
    val testDir = "./target"
    val classFiles = countFilesByFileType(testDir, ".class")
    //assert(classFiles.length > 0)

    // Uncomment to print to console
    // for (file <- classFiles) println(file)
  }
